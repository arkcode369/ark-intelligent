...
// makeHistoryWithLevFund builds a synthetic COT history (newest-first) where
// LevFund has a large bullish WoW change in the most recent record.
func makeHistoryLevFund(n int, spike float64) []domain.COTRecord {
	records := make([]domain.COTRecord, n)
	base := 10000.0
	for i := n - 1; i >= 0; i-- {
		records[i] = domain.COTRecord{
			LevFundLong:      base + float64(n-1-i)*100,
			LevFundShort:     base - float64(n-1-i)*50,
			DealerLong:       base - float64(n-1-i)*30,
			DealerShort:      base + float64(n-1-i)*80,
			ManagedMoneyLong: base + float64(n-1-i)*60,
			ManagedMoneyShort: base,
			SwapDealerLong:   base,
			SwapDealerShort:  base + float64(n-1-i)*40,
			AssetMgrLong:     base,
			AssetMgrShort:    base,
		}
	}
...
func TestComputeCategoryZScore_FlatHistory(t *testing.T) {
	...
	// zscore may be 0 or NaN (stddev=0 → guarded → returns 0)
...
func TestComputeCategoryZScore_SpikeProducesAlert(t *testing.T) {
	// Large spike should produce |zscore| >= 2.0
...
func TestComputeAllCategoryZScores_PopulatesAll(t *testing.T) {
	...
	// All z-scores should be non-zero since history has variation
...
func TestDetectCategoryDivergence_SpecVsCommercial(t *testing.T) {
	// LevFund bullish, Dealer bearish → divergence
...
func TestDetectCategoryDivergence_NoDivergence(t *testing.T) {
	// All in same direction → no divergence
	a := &domain.COTAnalysis{
		LevFundZScore:      1.0,
		DealerZScore:       0.5,
